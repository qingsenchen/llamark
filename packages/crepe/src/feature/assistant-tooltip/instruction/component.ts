...
export interface InstructionProps {
  ...
  type: string
  hide: () => void
  config?: AssistantTooltipFeatureConfig
}

export const instructionComponent: Component<InstructionProps> = ({
  show,
  hide,
  ctx,
  filter,
  ...
  const { groups, size } = useMemo(() => getGroups(filter, type, config), [type])
...
export const InstructionElement = c(instructionComponent)
