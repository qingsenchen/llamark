...
import type { MathComponentProps } from './component'
import type { LanguageLoader } from './loader'

export class LlamarkMathBlock implements NodeView {
  dom: HTMLElement & MathComponentProps
  cm: CodeMirror
  ...
  private forwardUpdate = (update: ViewUpdate) => {
    ...
    if (update.docChanged || pmSel.from !== selFrom || pmSel.to !== selTo) {
      ...
      this.dom.content = update.state.doc.toString()
    }
  }

  private createDom() {
    const dom = document.createElement('llamark-math-block') as HTMLElement & MathComponentProps
    dom.codemirror = this.cm
    dom.getAllLanguages = this.getAllLanguages
    dom.setLanguage = this.setLanguage
    dom.setFocused = this.setFocused
  ...
  update(node: Node) {
    ...
    this.node = node
    this.dom.focused = node.attrs.focused
    this.updateLanguage()
  ...
  setFocused = (focus: boolean) => {
    this.view.dispatch(
      this.view.state.tr.setNodeAttribute(this.getPos() ?? 0, 'focused', focus),
    )
  }
